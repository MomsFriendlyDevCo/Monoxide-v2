...
module.exports = function MonoxideScenario(o, input, options) {
	return Promise.resolve()
		...
		.then(blob => {
			_.forEach(blob, (v, k) => {
				if (!o.collections[k]) throw new Error(`Unknown model "${k}" when prepairing to create scenario`);
			});
			return blob;
		})
		.then(blob => {
			...
			return Promise.all(
				_.keys(blob)
					.map(m => Promise.resolve()
						.then(()=> o.collections[m].deleteMany())
					)
			).then(()=> blob);
		})
		...
		.then(blob => {
			...
			var tryCreate = ()=>
				o.utils.promiseThrottle(options && options.threads ? options.threads : 3, queue.map(item => ()=> {
					...
					if (!o.collections[item.collection]) throw new Error(`Cannot create item in non-existant or model "${item.collection}"`);
					return o.collections[item.collection].create(item.item)
						.then(created => {
							// Stash ID?
							if (item.id) lookup[item.id] = created[o.collections[item.collection].settings.idField];
							item.created = true;
