...
* @property {function} remove A function that can be called as `()` to remove this node from the output
*/
...
* @param {boolean} [options.arrays=true] Also call endpoint function on arrays (and their child values)
			...
			if (schemaContext[schemaKey] === undefined) { // Reached endpoint
				...
				if (settings.arrays) {
					node = {
						...
						replace: val => {
							node.value = val;
							return node.doc.$set(node.docPath, val);
						},
						remove: val => node.doc.$unset(node.docPath),
					};
					waitingOn.push(func.call(node, node));
				}

				if (!schemaContext[schemaKey].arrayType || schemaContext[schemaKey].arrayType == 'scalar') { // Array of primative scalars
					var arrayContext = docContext && docContext[schemaKey] ? docContext[schemaKey] : [];
					if (!_.isArray(arrayContext)) {
						if (debug.enabled) debug('Attmpting to update array-of-primatives at path', schemaPath.concat([schemaKey]).join('.'), 'with non-array object', arrayContext);
						throw new Error(`Cannot update array-of-primatives at path ${schemaPath.concat([schemaKey]).join('.')} with non-array object`);
					}

					arrayContext.forEach((arrayVal, arrayIndex) => { // Iterate over each data array item
					...
					var arrayContext = docContext && docContext[schemaKey] ? docContext[schemaKey] : [];
					if (!_.isArray(arrayContext)) {
						if (debug.enabled) debug('Attmpting to update collection array at path', schemaPath.concat([schemaKey]).join('.'), 'with non-array object', arrayContext);
						throw new Error(`Cannot update collection array at path ${schemaPath.concat([schemaKey]).join('.')} with non-array object`);
					}
