...
* @returns {Promise <MonoxideDocument>} A promise which will eventually resolve with the MonoxideDocument
*/
module.exports = function MonoxideDocument(o, collection, data, options) {
	...
	Object.defineProperties(doc, {
			...
			*/
			value() {
				return collection.emit('save', doc)
					.then(()=> doc.$toObject())
					.then(payload => {
						debugDetail('Create document', collection.name, '/', doc._id || '(No ID)', payload);
			...
			*/
			value(patch) {
				...
				return collection.emit('save', doc)
					...
					.then(payload => {
						if (!doc._id) throw new Error('Cannot update document with missing _id key');
						debugDetail('Saving document', collection.name, '/', doc._id, payload);
						return collection.mongoCollection.updateOne(
							{_id: _.isString(doc._id) ? ObjectID(doc._id) : doc._id}, // Mutate into an ObjectID if its not already
							{$set: payload},
						);
					})
					.then(()=> collection.emit('saved', doc))
		...
		$clone: {
			...
			* @returns {Promise <MonoxideDocument>} A Promise which will resolve with a new document instance
			*/
			value() {
				return new MonoxideDocument(o, collection, _.cloneDeep(doc), {
		...
		$toObject: {
			enumerable: false,
			/**
			* Convert the curent document to a plain object
			...
			*/
			value(patch) {
				return doc.$clone()
					.then(workDoc => workDoc.$setMany(patch))
					.then(workDoc => collection.emit('resolve', workDoc).then(()=> workDoc))
					.then(workDoc => workDoc.$each(node =>
						collection.emit('resolveNode', node)
							.then(()=> node.schema.type && collection.emit(`resolveNode:${node.schema.type}`, node))
					).then(()=> workDoc))
					.then(workDoc => collection.emit('resolved', workDoc).then(()=> workDoc))
					.then(workDoc => _.toPlainObject(workDoc))
			},
		},
		// }}}
		// $each {{{
				...
				if (path === undefined && val === undefined) { // Do nothing
					...
					return Promise.all(
						Object.keys(path).map(key =>
							doc.$each(key, node => node.replace(path[key]))
						)
					).then(()=> doc);
		...
		$isModified: {
			enumerable: false,
			/**
			* Return if the entire document has any modifications (no params) or a specific path
			* NOTE: Unlike $each() this does not recurse down complex endpoints, only simple paths
			* @param {string|array} [path] If specified check only a specific path for modifications
			* @returns {boolean} Whether either the entire document or the specific path was modified
			*/
			value(path) {
				if (!path) {
					return _.isEqual(_.toPlainObject(doc), _.toPlainObject(data));
				} else { // Check specific path
					return (doc.$get(path) == _.get(data, path));
				}
			},
		},
		// }}}
	...
	if (!options || options.emitters == undefined || options.emitters) {
		return collection.emit('doc', doc)
			.then(()=> doc.$each(node =>
				collection.emit('docNode', node)
					.then(()=> node.schema.type && collection.emit(`docNode:${node.schema.type}`, node))
			)) // Trap array types also, so we can handle them as recursive types (apply defaults, validate etc.)
