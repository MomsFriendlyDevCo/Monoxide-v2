...
module.exports = function MonoxideDocument(o, collection, data, classProperties) {
	...
	Object.defineProperties(doc, {
		...
		save: {
			...
			*/
			value(patch) {
				...
				debug('Saving document', collection.name, '/', this._id);

				return this.toObject()
					.then(payload => {
						debugDetail('Saving document', collection.name, '/', this._id, payload);
						return collection.updateOneById(this._id, payload);
					});
			},
		},
		// }}}
		...
		delete: {
			...
			*/
			value() {
				return collection.deleteOneById(this._id);
			},
		},
		...
		$each: {
			...
			*/
			value(path, func) {
				...
				traverse(0, [], this, [], collection.schema);
	...
	// Virtual allocation {{{
	Object.keys(collection.virtuals).forEach(virtual => {
		if (collection.virtuals[virtual].getter) {
			debugDetail('Allocate virtual', virtual);
			doc.$set(virtual, collection.virtuals[virtual].getter(doc));
		}
	});
	// }}}
