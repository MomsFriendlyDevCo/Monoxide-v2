...
module.exports = function MonoxideCollection(o, name, schema) {
	...
	*/
	...
	/**
	* Handle for the Mongo collection when the schema has been compiled via c.createTable()
	* @var {MongoCollection}
	*/
	c.mongoCollection;
	...
	* @param {boolean} [removeMonoxide=true] If true the collection is also removed from the `monoxide.collections` lookup object
	* @returns {Promise} A promise which will resolve when the collection has been removed
	*/
	c.dropCollection = (removeMonoxide = true) => Promise.resolve()
		.then(()=> debug('Remove collection', c.name))
		.then(()=> c.mongoCollection.drop())
		.then(()=> debug('Dropped collection', c.name))
		.then(()=> removeMonoxide && delete o.collections[c.name])
	// }}}
