...
module.exports = function MonoxideQueryBuilder(o, collection) {
	...
	q.getAggregationQuery = ()=> {
		...
		if (debugQuery.enabled)
			debugQuery(
				...
				+ (!_.isEmpty(q.$sort) ? `\t.select(${JSON.stringify(q.$sort)})\n` : '')
				+ (q.$skip ? `\t.skip(${q.$skip})\n` : '')
	...
	*/
	q.getAggregationCursor = ()=>
		collection.aggregate(q.getAggregationQuery(), q.cursorOptions)
			.then(cursor => new o.classes.Cursor(o, collection, cursor))
			.then(cursor => {
	...
	*/
	q.exec = ()=> {
		...
		if (q.$as == 'slurp') {
			...
			setTimeout(()=> {
				Promise.resolve(q.getAggregationCursor())
